...
def analyse_item_data(item):
    #Lowest listings
    print('Previous lowest_listing length is ' + len(item.lowest_listings).__str__())
    item = exclude_listings_from_item(item)
    print('New lowest_listing length is ' + len(item.lowest_listings).__str__())
    analyse_lowest_listing_data(item)
